...
# Step 2: PyInstaller
run("python -m PyInstaller YiseAssistant.spec --noconfirm", desc="[2/3] PyInstaller 打包 exe")

# Step 3: zip dist
dist_dir = os.path.join(ROOT, "dist", "瑞玛丽小助手")
zip_path = os.path.join(ROOT, "dist", "瑞玛丽小助手_V1.0.zip")
if os.path.exists(zip_path):
   os.remove(zip_path)
print(f"\n{'='*50}")
print(f"  [3/3] 压缩 dist 文件夹")
print(f"{'='*50}\n")
shutil.make_archive(zip_path.replace(".zip", ""), "zip", os.path.join(ROOT, "dist"), "瑞玛丽小助手")
if os.path.exists(zip_path):
   print(f"\n[OK] 压缩完成: {zip_path}")
